...
- **Tests de frontend (formularios críticos).** Agregar tests con Vitest + React Testing Library para los formularios con lógica no trivial: login (validación de campos, manejo de error del servidor), formulario de alumno (campos requeridos, feedback de error). Los componentes de solo presentación (tablas, badges) tienen poco valor de test unitario — el foco sería en la interacción del usuario con los formularios y los mensajes de error. *(evaluado: 2026-07-18)*
